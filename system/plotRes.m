...
for l=1:length(mixs)
    ...
    for g=1:length(W)
        ...
        exportgraphics(gcf,sprintf("img/sinResOpt%d_%s.png",optRes.w(1),mix))
        close()
        ...
        exportgraphics(gcf,sprintf("img/sinResGA%d_%s.png",optRes.w(1),mix))
        close()
        ...
        grid on;
        box on;
        stairs(X,optRes.NC(1:dsampling:size(optRes.Tsim,2),:),'linewidth',2)
        xlabel("Time(s)")
        ylabel("#Cores")
        legend("Frontend","Catalog Svc","Carts Svc")
        %export_fig(sprintf("img/sinUOpt%d_%s.pdf",optRes.w(1),mix))
        exportgraphics(gcf,sprintf("img/sinUOpt%d_%s.png",optRes.w(1),mix))
        close()
        ...
        grid on;
        box on;
        stairs(X,gaRes.NC(1:dsampling:size(optRes.Tsim,2),:),'linewidth',2)
        xlabel("Time(s)")
        ylabel("#Cores")
        legend("Frontend","Catalog Svc","Carts Svc")
        %export_fig(sprintf("img/sinUGA%d_%s.pdf",optRes.w(1),mix))
        exportgraphics(gcf,sprintf("img/sinUGA%d_%s.png",optRes.w(1),mix))
        close()
        ...
        exportgraphics(gcf,sprintf("img/sinObj%d_%s.pdf",optRes.w(1),mix))
        close()
