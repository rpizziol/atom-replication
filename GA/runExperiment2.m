...
%     options = optimoptions(options,'PlotFcn', {@gaplotbestf, @gaplotbestindiv});
    ...
    [], [], [], params.s_lb, params.s_ub, [], [], options); %aggiungre funzione nl per le constraint sui tempi di risposta non lineari
    %[], [], [], params.s_lb, params.s_ub, @(x)nlcon(x,model), [], options);
