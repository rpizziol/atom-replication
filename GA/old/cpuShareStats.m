...
objAvg = zeros(1, nexperiments);
avgCpuShare = zeros(4, nexperiments);
...
for i = 1:length(mixs)
    for j = 1:length(users)
        ...
        for k = 1:size(filenames, 1)
            ...
            sumobj = sumobj + bestValues(end);
            sumCpuShares = sumCpuShares + bestIndividuals(end, :);
        ...
        objAvg(expId) = sumobj / size(filenames, 1);
        avgCpuShare(:,expId) = sumCpuShares / size(filenames, 1);
